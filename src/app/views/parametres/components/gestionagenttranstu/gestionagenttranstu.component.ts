...
@Component({
  selector: 'app-gestionagenttranstu',
  templateUrl: './gestionagenttranstu.component.html',
  styleUrls: ['./gestionagenttranstu.component.css']
})
export class GestionAgentTranstuComponent implements OnInit {
  pageTitle: string;
  error: string;
  uploadError: string;
  wcode : string = '';
  ...
  infoForm() {
    this.crudApi.dataForm = this.fb.group({
        id: [''],
        function: ['', [ RxwebValidators.compose({
          validators:[
    ...
    const agenttranstu = this.crudApi.dataForm.value;
      ...
      if (id) {
        this.crudApi.updatedata(+id,agenttranstu).subscribe( data => {
          ...
          this.initForm();
          this.router.navigate(['/home/parametres/gestionagenttranstu']); 
        });
      } else {
