import { Component, OnInit ,Inject} from '@angular/core';
import { DepotService} from '../../service/depot.service'
import { ToastrService } from 'ngx-toastr';
import { FormBuilder, FormGroup, FormControl, ReactiveFormsModule,Validators }
from '@angular/forms';
import { Router ,ActivatedRoute} from '@angular/router';
import { Depot} from '../../model/depot';
import { RxwebValidators } from '@rxweb/reactive-form-validators';

@Component({
  selector: 'app-gestiondepot',
  templateUrl: './gestiondepot.component.html',
  styleUrls: ['./gestiondepot.component.css']
})
export class GestionDepotComponent implements OnInit {
  public depots;
  pageTitle: string;
  error: string;
  uploadError: string;
  wcode : string = '';
  public message: string;
  submitted = false;
  page1=1 ;
  motcle1 = '';
  count1 ;
   pageSize1 = 2;
  constructor(public crudApi: DepotService ,public fb: FormBuilder,public toastr: ToastrService,
    private router : Router,private route: ActivatedRoute) { }
    isCollapsed1: boolean = false;
  isCollapsed2: boolean = false;
  iconCollapse: string = 'icon-arrow-up';
  isClose1: boolean = false;
  isClose2: boolean = false;
  collapsed(event: any): void {
    // console.log(event);
  }

  expanded(event: any): void {
    // console.log(event);
  }

  toggleCollapse1(): void {
    this.isCollapsed1 = !this.isCollapsed1;
    this.iconCollapse = this.isCollapsed1 ? 'icon-arrow-down' : 'icon-arrow-up';
  }
  toggleCollapse2(): void {
    this.isCollapsed2 = !this.isCollapsed2;
    this.iconCollapse = this.isCollapsed2 ? 'icon-arrow-down' : 'icon-arrow-up';
  }
  get f() { return this.crudApi.dataForm.controls; }
  ngOnInit() {
    const id = this.route.snapshot.paramMap.get('id');
    if (id) {
      this.pageTitle = 'Edit Depot';
      this.crudApi.getData(+id).subscribe(
        res => {
          this.crudApi.dataForm.patchValue({
            id:res.id,
            nameDepot: res.nameDepot,
          });
          
        }
      );
    } else {
      this.pageTitle = 'Create Depot';
    }
   this.infoForm();
    
    
  }
  
  retrieveDepot(): void {
    let p = this.page1-1;
    
    this.crudApi.getDepotsPagination( p, this.pageSize1).subscribe(
      response =>{this.depots = response;
        this.crudApi.getAll().subscribe(
         
          r =>{
            this.count1 =r.length;
          },err=>{
            console.log(err);
              }
         );
        
        
      },err=>{
        console.log(err);
          }
     );

  }
   
 
  infoForm() {
    this.crudApi.dataForm = this.fb.group({
      id: [''],
      nameDepot:  ['', [ RxwebValidators.compose({
        validators:[
        RxwebValidators.required(),
        RxwebValidators.pattern({expression:{'name': /^[a-zA-Z]+[ ]?([a-zA-Z0-9]+[ ]?)?[a-zA-Z0-9]+$/} })
        ]})]],
        });
    }
   
    initForm() {
      this.crudApi.dataForm = this.fb.group({
        id: [''],
        nameDepot: [''],
          });
      }

      getData() {
        this.crudApi.getAll().subscribe(
          response =>{this.depots = response ;}
         );
       
      }

  ResetForm() {
      this.crudApi.dataForm.reset();
  }
  onSubmit() {
    this.submitted = true;
    if (this.crudApi.dataForm.invalid) {
      return;
  }
    const depot = this.crudApi.dataForm.value;
  
      const id = this.crudApi.dataForm.get('id').value;
  
      if (id) {
        this.crudApi.updatedata(+id,depot).subscribe( data => {
          this.toastr.success( 'Validation Faite avec Success');
          this.retrieveDepot();
          this.initForm();
          this.router.navigate(['/home/parametres/gestiondepot']); 
        });
      } else {
        this.crudApi.createData(depot).subscribe( data => {
          this.toastr.success( 'Validation Faite avec Success');
          this.retrieveDepot();
          this.initForm();
          this.router.navigate(['/home/parametres/gestiondepot']);
        });
      }
}

onReset() {
  this.submitted = false;
  this.crudApi.dataForm.reset();
}

close1() {
  this.isClose1 = !this.isClose1;
  const card = document.getElementById('id1');
  if (this.isClose1 )
  {
  card.style.display = "none";
  
}
}
  
close2() {
  this.isClose2 = !this.isClose2;
  const card = document.getElementById('id2');
  if (this.isClose2 )
  {
  card.style.display = "none";
  
}
}

}
