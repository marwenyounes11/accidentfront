...
export class NbrBlesserInterneComponent implements OnInit, OnDestroy {
  nbrBlesserInterne;
  format;
  ...
  ngOnInit() {
    this.subscription= this.rapportStatistiqueService.getNbrBlesserInterne()
    .subscribe(data=>{
this.nbrBlesserInterne=data;
    },err=>{
console.log(err);
    })
  ...
  runExcel(){
    this.format="excel"
    this.rapportFormatService.getReportNbrBlesserInterne(this.format).subscribe(
      data => {
        console.log(data);
      },
