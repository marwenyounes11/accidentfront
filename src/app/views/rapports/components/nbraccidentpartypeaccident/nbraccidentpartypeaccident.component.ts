...
export class NbrAccidentParTypeAccidentComponent implements OnInit, OnDestroy {
  nbrAccidentParTypeAccident;
  format;
  ...
  ngOnInit() {
    this.subscription= this.rapportStatistiqueService.getNbrAccidentParTypeAccident()
    .subscribe(data=>{
...
console.log(err);
  ...
  runExcel(){
    this.format="excel"
    this.rapportFormatService.getReportNbrAccidentParTypeAccident(this.format).subscribe(
      data => {
        console.log(data);
      },
