...
export class NbrAccidentParDateComponent implements OnInit, OnDestroy {
  nbrAccidentParDate;
  format;
  ...
  ngOnInit() {
    this.subscription= this.rapportStatistiqueService.getNbrAccidentParDate()
    .subscribe(data=>{
...
console.log(err);
  ...
  runExcel(){
    this.format="excel"
    this.rapportFormatService.getReportNbrAccidentParDate(this.format).subscribe(
      data => {
        console.log(data);
      },
