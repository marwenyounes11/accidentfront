...
export class NbrAccidentParSousTypeAccidentComponent implements OnInit, OnDestroy {
  nbrAccidentParSousTypeAccident;
  format;
  ...
  ngOnInit() {
    this.subscription= this.rapportStatistiqueService.getNbrAccidentParSousTypeAccident()
    .subscribe(data=>{
...
console.log(err);
  ...
  runExcel(){
    this.format="excel"
    this.rapportFormatService.getReportNbrAccidentParSousTypeAccident(this.format).subscribe(
      data => {
        console.log(data);
      },
