...
export class NbrBlesserParNiveauBlessureComponent implements OnInit, OnDestroy {
  nbrBlesserParNiveauBlessure;
  format;
  ...
  ngOnInit() {
    this.subscription= this.rapportStatistiqueService.getNbrBlesserParNiveauBlessure()
    .subscribe(data=>{
this.nbrBlesserParNiveauBlessure=data;
    },err=>{
console.log(err);
    })
  ...
  runExcel(){
    this.format="excel"
    this.rapportFormatService.getReportNbrBlesserParNiveauBlessure(this.format).subscribe(
      data => {
        console.log(data);
      },
