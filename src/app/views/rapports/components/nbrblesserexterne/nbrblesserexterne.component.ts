...
export class NbrBlesserExterneComponent implements OnInit, OnDestroy {
  nbrBlesserExterne;
  format;
  ...
  ngOnInit() {
    this.subscription= this.rapportStatistiqueService.getNbrBlesserExterne()
    .subscribe(data=>{
this.nbrBlesserExterne=data;
    },err=>{
console.log(err);
    })
  ...
  runExcel(){
    this.format="excel"
    this.rapportFormatService.getReportNbrBlesserExterne(this.format).subscribe(
      data => {
        console.log(data);
      },
