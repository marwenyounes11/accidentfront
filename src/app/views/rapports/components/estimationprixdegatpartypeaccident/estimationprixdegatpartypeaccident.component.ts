...
export class EstimationPrixDegatParTypeAccidentComponent implements OnInit, OnDestroy {
  estimationPrixDegatParTypeAccident;
  format;
  ...
  ngOnInit() {
    this.subscription= this.rapportStatistiqueService.getEstimationPrixDegatParTypeAccident()
    .subscribe(data=>{
...
console.log(err);
  ...
  runExcel(){
    this.format="excel"
    this.rapportFormatService.getReportEstimationPrixDegatParTypeAccident(this.format).subscribe(
      data => {
        console.log(data);
      },
