...
@Component({
  selector: 'app-nbraccidentparmoyentransportdate',
  templateUrl: './nbraccidentparmoyentransportdate.component.html',
  styleUrls: ['./nbraccidentparmoyentransportdate.component.css']
})
export class NbrAccidentParMoyenTransportDateComponent implements OnInit, OnDestroy {
  nbrAccidentParMoyenTransportDate;
  format;
  ...
  ngOnInit() {
    this.subscription= this.rapportStatistiqueService.getNbrAccidentParMoyenTransportDate()
    .subscribe(data=>{
this.nbrAccidentParMoyenTransportDate=data;
    },err=>{
console.log(err);
    })
  ...
  runExcel(){
    this.format="excel"
    this.rapportFormatService.getReportNbrAccidentParMoyenTransportDate(this.format).subscribe(
      data => {
        console.log(data);
      },
