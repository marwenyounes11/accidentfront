...
export class EstimationPrixDegatParDateComponent implements OnInit, OnDestroy {
  estimationPrixDegatParDate;
 format;
  ...
  ngOnInit() {
    this.subscription= this.rapportStatistiqueService.getEstimationPrixDegatParDate()
    .subscribe(data=>{
...
console.log(err);
  ...
  runExcel(){
    this.format="excel"
    this.rapportFormatService.getReportEstimationPrixDegatParDate(this.format).subscribe(
      data => {
        console.log(data);
      },
