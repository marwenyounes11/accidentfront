...
export class NbrAccidentParMoyenTransportComponent implements OnInit, OnDestroy {
  nbrAccidentParMoyenTransport;
  format;
  ...
  ngOnInit() {
    this.subscription= this.rapportStatistiqueService.getNbrAccidentParMoyenTransport()
    .subscribe(data=>{
...
console.log(err);
  ...
  runExcel(){
    this.format="excel"
    this.rapportFormatService.getReportNbrAccidentParMoyenTransport(this.format).subscribe(
      data => {
        console.log(data);
      },
