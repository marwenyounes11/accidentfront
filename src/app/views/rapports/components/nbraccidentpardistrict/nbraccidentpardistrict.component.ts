...
export class NbrAccidentParDistrictComponent implements OnInit, OnDestroy {
  nbrAccidentParDistrict;
  format;
  ...
  ngOnInit() {
    this.subscription= this.rapportStatistiqueService.getNbrAccidentParDistrict()
    .subscribe(data=>{
...
console.log(err);
  ...
  runExcel(){
    this.format="excel"
    this.rapportFormatService.getReportNbrAccidentParDistrict(this.format).subscribe(
      data => {
        console.log(data);
      },
