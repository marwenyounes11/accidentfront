...
@Component({
  selector: 'app-add-accidentcollision',
  templateUrl: './add-accidentcollision.component.html',
  styleUrls: ['./add-accidentcollision.component.css']
})
export class AddAccidentCollisionComponent implements OnInit {
  pageTitle: string;
  error: string;
  uploadError: string;
  imagePath: String;
  ...
  ngOnInit(): void {
    ...
    {this.infoForm()
      this.pageTitle = 'Create Accident collision';
    } else {
      this.pageTitle = 'Update Accident collision';
    }
...
addData() {
  const formData = new  FormData();
    const collision= this.crudApi.dataForm.value;
    formData.append('collision',JSON.stringify(collision));
  ...
  updateData()
  {
    this.crudApi.updateAccidentCollision(this.crudApi.dataForm.value.id,this.crudApi.dataForm.value).subscribe( data => {
      this.toastr.success( 'Validation Faite avec Success');
      this.router.navigate(['/base/accidents']); 
    });
  }
