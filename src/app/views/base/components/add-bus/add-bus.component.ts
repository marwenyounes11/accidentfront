...
export class AddBusComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    {this.infoForm()
      this.pageTitle = 'Create Bus';
    } else {
      this.pageTitle = 'Update Bus';
    }
  }
...
addData() {
  const bus = this.crudApi.dataForm.value;
  ...
  updateData()
  {
    this.crudApi.updateBus(this.crudApi.dataForm.value.id,this.crudApi.dataForm.value).subscribe( data => {
      this.toastr.success( 'Validation Faite avec Success');
      this.router.navigate(['/base/moyentransports']); 
    });
  }
